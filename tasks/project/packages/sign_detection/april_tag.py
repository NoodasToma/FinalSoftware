...
Ported from KvatiTown. ONE adaptation for this project: the reference swapped tag
IDs 10<->11 ("the bot reads 10 and 11 backwards") — that is a quirk of THEIR
hardware/printed signs. Our camera reads tags correctly (confirmed on the real bot's
web UI) AND the Godot sim renders tag textures correctly, so for us the swap is WRONG
(it would invert left-T <-> T-intersection). It is therefore OFF by default here.
Set SWAP_10_11 = True only if a specific camera genuinely reads 10/11 backwards.
"""
...
# See the module docstring. False = no swap (correct for our sim + our bot).
SWAP_10_11 = False
